...
public class server extends JHTTPServer
{
    ...
    public Response serve(String uri, String method, Properties header, Properties parms, Properties files) {
         ...
         else if(uri.equals("/change_password"))
        {
            ...
            String username=parms.getProperty("username");
            String new_password=parms.getProperty("new_password");
            ...
            try
            {
                ...
                else
                {
                     ...
                     return new Response(HTTP_OK, "text/plain", "username does not exist");
                }
            }
            catch (Exception e) 
            {
        ...
        else if(uri.equals("/mark_attendance"))
        {
            ...
            try
            {
                ResultSet rs=DBLoader.executeQuery("select * from attendance");
                
                    
                    rs.moveToInsertRow();
                    rs.updateString("student_email",email);
                    rs.updateString("date_of_attendance",date);
        ...
        {
            int assignment_id=Integer.parseInt(parms.getProperty("assignment_id"));
            String student_email=parms.getProperty("student_email");
            ...
            try
            {
                ResultSet rs=DBLoader.executeQuery("select * from submit_assignment where assignment_id="+assignment_id);
                if(rs.next())
                {
                    return new Response(HTTP_OK, "text/plain", "already submitted");
                }
                else
                {
...
//                   
         ...
         else if(uri.equals("/fetch_questions"))
        {
           
            try
            { 
               
                ResultSet rs=DBLoader.executeQuery("select * from chat where answer='na'");
                String ans="";
                 while(rs.next())
                {
                   
                    String id=rs.getString("chat_id");
                    String question=rs.getString("question");
                    ans=ans+id+"$"+question+";;";
//                   
                }
...
//                   
        ...
        if(uri.equals("/fetch_assignment"))
        {
            int course1=Integer.parseInt(parms.getProperty("course"));
            String semester1=parms.getProperty("semester");
           try
            {
                ResultSet rs=DBLoader.executeQuery("select * from assignment where semester='"+semester1+"' and course="+course1);
                String ans="";
               
                    while(rs.next())
                    {
                    String asid=rs.getInt("asid")+"";
                    ResultSet rs1=DBLoader.executeQuery("select * from submit_assignment where assignment_id="+rs.getInt("asid"));
                    String teacher_id=rs.getString("teacher_id");
                   ...
                   String course=rs.getInt("course")+"";
                    ...
                    {
                        ans=ans+asid+"$"+teacher_id+"$"+title+"$"+detail+"$"+date_time+"$"+course+"$"+semester+"$"+file+"$"+lastdate+";";
                    }
                    }
                    System.out.println(ans);
...
//                   
        ...
        else if(uri.equals("/fetch_selected_student_data_based_on_course"))
        {
           ...
           String semester1=parms.getProperty("semester");
            ...
            { 
                ...
                ResultSet rs=DBLoader.executeQuery("select * from students where semester='"+semester1+"'and course="+course1);
                String ans="";
...
//                   
        ...
        else if(uri.equals("/check_assignment_data"))
        {
             System.out.println("hello");
          
            try
            { 
                ResultSet rs=DBLoader.executeQuery("select * from submit_assignment ");
                String ans="";
                 ...
                 while(rs.next())
                {
                    ...
                    if(grade.equals("na"))
                    {
                         ans=ans+id+"$"+assignment_id+"$"+student_email+"$"+dateofsubmit+"$"+file+"$"+grade+";";
        ...
        if(uri.equals("/add_time_table"))
        {
            ...
            try
            {
                ResultSet rs=DBLoader.executeQuery("select * from timetable where semester='"+semester+"' and course="+course);
                if(rs.next())
                {
                    return new Response(HTTP_OK, "text/plain", "exist");
                }
                ...
                {
                    String photoname=saveFileOnServerWithRandomName(files, parms, "file", "src/upload_time_table/");
                    rs.moveToInsertRow();
                    rs.updateInt("course",course);
                    rs.updateString("semester",semester);
                    rs.updateString("teacher_id",teacher_id);
                    rs.updateString("file","src/upload_time_table/"+photoname);
                    rs.insertRow();
        ...
        if(uri.equals("/post_answer"))
        {
            int chat_id=Integer.parseInt(parms.getProperty("chat_id"));
            ...
            try
            {
                ResultSet rs=DBLoader.executeQuery("select * from chat where chat_id="+chat_id);
                if(rs.next())
                {
                     rs.updateString("teacher_email",email);
                ...
                else
                {
                     ...
                     return new Response(HTTP_OK, "text/plain", "wrong question id");
                }
            }
            catch (Exception e) 
            {
        ...
        if(uri.equals("/post_grade"))
        {
            int assignment_id=Integer.parseInt(parms.getProperty("assignment_id"));
            
            String email=parms.getProperty("email");
            String grade=parms.getProperty("grade");
            ...
            try
            {
                ResultSet rs=DBLoader.executeQuery("select * from submit_assignment where student_email='"+email+"' and assignment_id="+assignment_id);
                if(rs.next())
                {
                ...
                else
                {
                     ...
                     return new Response(HTTP_OK, "text/plain", "answer does not exist");
                }
            }
            catch (Exception e) 
            {
        ...
        if(uri.equals("/add_assignment"))
        {
            ...
            String last_date=parms.getProperty("last_date");
           
            
            
            try
            {
                ResultSet rs=DBLoader.executeQuery("select * from assignment where title='"+title+"'");
                if(rs.next())
                {
                    return new Response(HTTP_OK, "text/plain", "exist");
                }
                ...
                {
                    String photoname=saveFileOnServerWithRandomName(files, parms, "file", "src/upload_assignment/");
                    rs.moveToInsertRow();
                    rs.updateString("teacher_id",teacher_id);
                    ...
                    rs.updateInt("course",course);
                    rs.updateString("semester",semester);
                    rs.updateString("file","src/upload_assignment/"+photoname);
                     rs.updateString("lastdate",last_date);
                    rs.insertRow();
