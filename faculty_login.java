...
public class faculty_login extends javax.swing.JFrame {
    ...
    public faculty_login() {
        ...
        try (FileInputStream out = new FileInputStream("config.properties")) 
        {
            prop.load(out);
            String fac=prop.getProperty(name);
            if(fac!=null)
            {
                faculty_home obj=new faculty_home(fac);
                obj.setVisible(true);
                setVisible(false);
            }
        } 
    ...
    private void otpbtActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_otpbtActionPerformed
        ...
        if(ans.equals(value))
        {
            ...
            String name="faculty_email";
            AddSession obj=new AddSession(email,name);
            obj.func();
            faculty_home obj2=new faculty_home(email);
        }
        else
        {
